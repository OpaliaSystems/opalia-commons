...
class CryptoTest
  ...
  it should "sign SHA-512 with correct result" in {
    Hex.encode(Crypto.Digester.SHA512.sign("this is a test")) should
  ...
  it should "sign MD5/HMAC with correct result" in {
    Hex.encode(Crypto.DigesterHMAC.MD5.sign("this is a test", "secret key")) should
      be("c51fe3378048cfdc356a902e2236e6b3")
  }

  it should "sign SHA-1/HMAC with correct result" in {
    Hex.encode(Crypto.DigesterHMAC.SHA1.sign("this is a test", "secret key")) should
      be("6cf81c2c82bb336d968bc6e3aba76f13ce5a3fe8")
  }

  it should "sign SHA-256/HMAC with correct result" in {
    Hex.encode(Crypto.DigesterHMAC.SHA256.sign("this is a test", "secret key")) should
      be("f3646df7b4ce412dd952bd3be01524237c7e18ef07ac286c2b19b97ef8d0e1f3")
  }

  it should "sign SHA-384/HMAC with correct result" in {
    Hex.encode(Crypto.DigesterHMAC.SHA384.sign("this is a test", "secret key")) should
  ...
  it should "sign SHA-512/HMAC with correct result" in {
    Hex.encode(Crypto.DigesterHMAC.SHA512.sign("this is a test", "secret key")) should
  ...
  it should "encrypt/decrypt DES with correct result" in {

    val secret = "password"
    val message = "this is a secret message"

    val encrypted =
      Crypto.Cipher.DES.encrypt(message, secret)
  ...
  it should "encrypt/decrypt DESede with correct result" in {
    ...
    val encrypted =
      Crypto.Cipher.DESede.encrypt(message, secret)
  ...
  it should "encrypt/decrypt AES with correct result" in {
    ...
    val encrypted =
      Crypto.Cipher.AES.encrypt(message, secret)
  ...
  it should "encrypt/decrypt AES using InputStream with correct result" in {
    ...
    def readAllBytes(inStream: InputStream): Array[Byte] = {
  ...
  it should "encrypt/decrypt AES with failure for different keys" in {
    ...
    val secret1 = "password"
    val secret2 = "incorrect password"
    val message = "this is a secret message"

    val encrypted =
      Crypto.Cipher.AES.encrypt(message, secret1)
