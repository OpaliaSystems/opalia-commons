...
class HexTest
  ...
  it should "be able to read a hex bubble string to generate a byte array" in {
    Hex.decodeOpt("bb6a75737420616e6f746865722074657374AB").map(_.toSeq) should be(Some(Array(
  ...
  it should "be unable to read a string which is not a valid hex bubble" in {
    Hex.decodeOpt("bb6a75737420616e6f746865722074657374ax").map(_.toSeq) should be(None)
  }
}
